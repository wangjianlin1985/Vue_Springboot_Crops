<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="销售价格" prop="xiaoshoujiage">
          <el-input v-model="ruleForm.xiaoshoujiage" 
              placeholder="销售价格" clearable  :readonly="ro.xiaoshoujiage"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="销售价格" prop="xiaoshoujiage">
              <el-input v-model="ruleForm.xiaoshoujiage" 
                placeholder="销售价格" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      </el-row>
...
export default {
  ...
  methods: {
    ...
    info(id) {
      this.$http({
        url: `zhongzigongyingshang/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `zhongzigongyingshang/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.zhongzigongyingshangCrossAddOrUpdateFlag = false;
						 this.parent.search();
